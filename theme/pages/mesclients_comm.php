...
if(isset($_POST['deleteme']) && isset($_POST['liste']) )
{
 ...
 $zero=0 ;
	foreach($ed_selectionne as $key=>$value)
	{
		$myadmin->ClientDeleteRequest($_SESSION['comm'],$value);
...
if (isset($_GET['interesse'])) {
    $mesclient = $myadmin->ClientInteresseParCommercial($_GET['interesse'], $_SESSION['comm']);
} else if (($_GET['activite'] == 0 && $_GET['ville'] != 0) || ($_GET['activite'] != 0 && $_GET['ville'] == 0)) {
    if ($_GET['activite'] == 0) {
        $mesclient = $myadmin->FiltreParVille($_GET['ville'], $_SESSION['comm']);
        ...
        $mesclient = $myadmin->FiltreParCategorie($_GET['activite'], $_SESSION['comm']);
        $act = $myadmin->AfficherSecteur($_GET['activite']);
        $vil = $myadmin->NomVille($_GET['ville']);
        $filtre = 'les client de secteur : ' . $act;
    }
} else if (isset($_GET['activite']) && isset($_GET['ville'])) {
    $mesclient = $myadmin->FiltreCategorieVille($_GET['activite'], $_GET['ville'], $_SESSION['comm']);

    $act = $myadmin->AfficherSecteur($_GET['activite']);
    $vil = $myadmin->NomVille($_GET['ville']);
    $filtre = $act . ' à ' . $vil;
} else {
    $mesclient = $myadmin->ListeClientsParCommercial($_SESSION['comm']);
}
...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            MES CLIENTS
            <small><a href="page_comm.php?part=mesclient">Tous </a></small>
        </h1>
        <ol class="breadcrumb">
		...
		<div class="row">
            <!-- left column -->
             <div class="col-xs-12">
                <!-- general form elements -->		
              <div class="box">
			  <form method="post" action="page_comm.php?part=mesclient">
            <div class="box-header">
              <h3 class="box-title">LISTE DE MES CLIENTS</h3>
            </div>
                <div class="box-body">
				  <div class="box-tools">
                <!-- Check all button -->
...
foreach ($mesclient as $mcls) {
    ...
    ?>	
                                <tr <?= $styletr ?> >
								   <td><input name="liste[]" type="checkbox" value="<?= $mcls['id_cl'] ?>"></td>
                                   <td><?= $nb ?> </td>
        ...
        <!-- /.modal -->
                                    ...
                                    <a href="editclient.php?idclient=<?= $mcls['id_cl'] ?>" target="_blank" ><button type="button" class="btn btn-block btn-success btn-flat"  alt="Edit"><i class="fa fa-pencil-square"></i></button></a>
                                    </td>
                                    <td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <a href="page_comm.php?part=mesclient&cdr=ok&c=<?= $mcls['id_cl'] ?>"> <button type="button" class="btn btn-danger btn-ok">Confirmé</button></a>
                </div>
            </div>
        </div>
    </div>  
